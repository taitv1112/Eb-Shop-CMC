...
public class AuthorServiceImpl implements AuthorService {
    ...
    public ResponseEntity<String> saveAuthor(AuthorDTO authorDTO) {
        ...
        Author author = new Author(authorDTO.getAuthorId(), authorDTO.getAuthorName(),authorDTO.getWebsite(),authorDTO.getOrganization(),0L);
    ...
    public ResponseEntity<?> getAuthorById(String id) {
        ...
        AuthorDTO authorDTO = authorRepository.findAuthorByAuthorId(AuthorDTO.class, id);
        AuthorAdnBookDTO authorAdnBookDTO = new AuthorAdnBookDTO();
        List<TopSellingBooks> book = bookService.find3MostSoldBook(authorDTO.getAuthorId());
    ...
    public void soldBook(List<OrderDetail> orderDetails) {
        for (OrderDetail orderDetail : orderDetails) {
            authorRepository.soldBook(orderDetail.getQuantity(),orderDetail.getBook().getAuthor().getAuthorId());
        }
    }
}
